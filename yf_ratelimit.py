...
class _CachedTicker:
    ...
    def _refresh_session(self):
        ...
        IMPORTANT: this must stay scoped to THIS wrapper's own Ticker object.
        An earlier version also reset yfinance's process-wide crumb/cookie
        singleton (yfinance.data.YfData._instance) here. That singleton is
        shared by every yf.Ticker() in the app, including plain unshielded
        calls elsewhere (current price, comp valuation) — so a retry in
        this wrapper for one symbol was silently breaking unrelated yfinance
        calls happening anywhere else in the same process. Do not reintroduce
        that global mutation; only touch this instance's own session/object.
        """
        with self._yf_lock:
            self._yf_obj = None
